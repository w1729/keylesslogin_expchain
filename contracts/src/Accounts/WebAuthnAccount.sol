...
import {Groth16Verifier} from "../verifier/verifier.sol";
...
contract WebAuthnAccount is BaseAccount, TokenCallbackHandler, UUPSUpgradeable, Initializable, Test {
  ...
  constructor(IEntryPoint anEntryPoint, address _webauthnVerifier, address _loginService, address _factory, address _verifier) {
  ...
  function addFirstSigner(bytes calldata credId, uint256[2] calldata pubKeyCoordinates,uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[1] calldata _pubSignals) external onlyFactory {
    require(_signersCount == 0, "first signer already set");
    require(Groth16Verifier(verifier).verifyProof(pubKeyCoordinates, _pA, _pB,  _pC,_pubSignals)==true,"invalid proof");
    _addSigner(credId, pubKeyCoordinates);
  }
  ...
  function _addFirstSignerFromLoginService(
    ...
    bytes32 payload = keccak256(
      abi.encode(bytes1(uint8(SignatureTypes.LOGIN_SERVICE)), _login, credId, pubKeyCoordinates)
    );
  ...
  function _validateSignature(
    ...
    SignatureTypes signatureType = SignatureTypes(
      userOp.signature.length > 0 ? uint256(uint8(userOp.signature[0])) : 0
    );
  ...
  function _validateWebAuthnWithLoginServiceSignature(
    ...
    console.log("_validateWebAuthnWithLoginServiceSignature", signatureVerified);
